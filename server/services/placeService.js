...
const placeService = {
  ...
  searchPlaces: async (query, location, radius) => {
    try {
      ...
      // Cache the results
      await redis.setex(cacheKey, CACHE_TTL, JSON.stringify(response.data.results));
  ...
  getPlaceDetails: async (placeId) => {
    try {
      ...
      // If not in cache, fetch from Google Places API
      const url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,formatted_address,geometry,photos,rating,types,vicinity,website,phone_number,opening_hours,reviews&key=${GOOGLE_MAPS_API_KEY}`;
      ...
      // Cache the results
      await redis.setex(cacheKey, CACHE_TTL, JSON.stringify(response.data.result));
